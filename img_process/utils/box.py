...
def box_step(img_src, w, h, step_row=1, step_col=1,letter_pace=14,row_space=25, is_process_link=True,is_get_main_region=True,try_num=''):
    ...
    while col_index< len(cols)-1:
        ...
        #print(space)
        #如果当前的space小于阈值，则检查之前的与之后的格子的space，哪个小，就把当前不符合规则的格子并入。BY DL
...
def get_cols(row_region,step_col,w,mehtod_avg=False,method_peak=False):
    ...
    col_index=0
    cols5 =[]
    #将cols3的各个col根据连续性，分为几段，放入cols4中 BY DL
...
def find_valley(data,step,bigsize=1):
    ...
    for i in range(0,data_size):
        ...
        ltnum = d_step
        #在d_step范围内，查找比i小的col的数量，比其小的数量大于90%的col，那么就是峰谷点————BY DL
        for s in range(1,step):
            left =0
            if i-s>0:
                left = i-s
        ...
        #对X于Y两个方向计算，如果与上一个valley点相差小于阈值，说明应该是重叠，更新下上一个点值，否则是新的valley点,加入到结果中
        if valley:
            if len(result)>0:
                last = result[len(result) - 1]
                if d.get("x"):
